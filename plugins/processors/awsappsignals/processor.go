...
func (ap *awsappsignalsprocessor) processMetrics(ctx context.Context, md pmetric.Metrics) (pmetric.Metrics, error) {
	...
	for i := 0; i < rms.Len(); i++ {
		...
		ap.logger.Info("8-3-4-2 DEBUG: ", zap.Any("Metric's Exponential Hist Attributes: ", ilms.At(0).Metrics().At(2).ExponentialHistogram().DataPoints().At(0).Attributes().AsRaw()))
