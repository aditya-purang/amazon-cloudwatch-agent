...
func (ap *awsappsignalsprocessor) processMetrics(ctx context.Context, md pmetric.Metrics) (pmetric.Metrics, error) {
	...
	for i := 0; i < rms.Len(); i++ {
		...
		ap.logger.Info("8-3-5 DEBUG: ", zap.Any("Metric Summary ", ilms.At(0).Metrics().At(0).Summary()))
		ap.logger.Info("8-3-6 DEBUG: ", zap.Any("Metric Summary ", ilms.At(0).Metrics().At(0).Gauge()))
		ap.logger.Info("8-3-7 DEBUG: ", zap.Any("Metric Summary ", ilms.At(0).Metrics().At(0).Sum()))
		ap.logger.Info("8-3-8 DEBUG: ", zap.Any("Metric Summary ", ilms.At(0).Metrics().At(0).Histogram()))
		ap.logger.Info("8-3-9 DEBUG: ", zap.Any("Metric Summary ", ilms.At(0).Metrics().At(0).Description()))
